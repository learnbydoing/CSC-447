...
object fp1 {
  ...
  def sum (xs : List[Int]) : Int = {
   ...
   xs match {
      case Nil => return 0
      case y::ys => if((y::ys).length == 1) y else y + sum(ys)
    }
  }
  ...
  // hence the "NoSuchElementException".
  def max (xs : List[Int]) : Int = {
    ...
    if(xs.head > maxSoFar) xs.head else maxSoFar
    }
  }
  ...
  def maxTailRecursiveAux (accumulator : Int, xs : List[Int]) : Int = {
    ...
    xs match {
    case Nil => accumulator 
    case x::tail => val newMax = if(x > accumulator) x else accumulator
    maxTailRecursiveAux(newMax, xs.tail)
    }
  }
