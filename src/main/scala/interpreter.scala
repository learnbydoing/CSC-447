...
object interpreter {
  ...
  def prettyPrintExp (e:Exp) : String = e match {
    ...
    // TODO: Add case for ExpIf below.
    case ExpIf(b, tt, ff) => "(if (" + prettyPrintExp(b) + ") "  +  prettyPrintExp(tt) + " else " + prettyPrintExp(ff)
  }
  ...
  val exXFunPlusFifty:Exp = ExpAbs("x", ExpApp (ExpApp (ExpVal(primAdd), ExpVar("x")), ExpVal(ValInt(50))))
  ...
  val exPair:Exp = ExpPair(ExpVal(ValInt(5)), ExpAbs("x", ExpVal(ValInt(6))))
  ...
  // val counterTest2:Exp = ExpLet ("counter2", ExpApp (counter, ExpVal (ValNil)), ExpSnd (ExpVar ("counter2")))   
  // val ct3:Exp = ExpApp(counterTest2, ExpVal(ValNil))   
  // val ct4 = ExpFst(ct3)
  // val exCounter:Exp = ct4   

  //val counter:Exp = ExpApp (counterFunction, ExpVal (ValInt (4)))

   val exCounter2 : Exp = ExpFst(
                      	      ExpApp(
                              ExpLet(
                              "counter2", 
                              ExpApp (counter, ExpVal (ValNil)), 
		              ExpSnd (ExpVar ("counter2"))
		              ), //end ExpLet
	                     ExpVal(ValNil)
	                      )
	                     )

   val exCounter : Exp = ExpFst(
                           ExpApp (
	                    ExpSnd(ExpApp(counter, ExpVal (ValNil))) , ExpVal(ValNil) 
                             )
                            )
...
counterFunction takes an integer and returns an anonymous function (as well as the integer that was passed to it). The
anonymous function that is returned takes an unused value and returns a pair consisting of n and the result of running 
counterFunction on (n+1), where n is the number passed to counterFunction.

The function counterFunction is shown below, it uses a closure (i.e. ExpAbs):

   val counterFunction:Exp = 
    ExpRecAbs (
      "counterFunction",
      "n",
      ExpAbs ( "m", ExpPair (ExpVar ("n"),ExpApp (ExpVar ("counterFunction"), ExpApp ( ExpApp (ExpVal (primAdd),ExpVar ("n")),ExpVal (ValInt (1))))))
	  )

The function counterFunctionBROKEN2 is the same as counterFunction, except that ExpAbs is replaced with ValAbs, so a closure is not used:

    val counterFunctionBROKEN2:Exp = 
    ExpRecAbs (
      "counterFunctionBROKEN2",
      "n",
      ExpVal ( ValAbs( "m", ExpPair (ExpVar ("n"), ExpApp (ExpVar ("counterFunctionBROKEN2"),ExpApp (ExpApp (ExpVal (primAdd), ExpVar ("n")),ExpVal (ValInt (1)))))))
    )
 
The following shows an application of the value 4 to the respective functions:
 
  val counter:Exp = ExpApp (counterFunction, ExpVal (ValInt (4)))   
  val ctrBROKEN2:Exp = ExpApp(counterFunctionBROKEN2, ExpVal(ValInt(4))) 
 
Then counter evaluates to:
 
  RESULT: [Map(n -> ValInt(4), counterFunction -> ValRecClosure(Map(),counterFunction,n,ExpAbs(m,ExpPair(ExpVar(n),ExpApp(ExpVar(counterFunction),ExpApp(ExpApp(ExpVal(ValPrimFun(Some(_+_),<function1>)),ExpVar(n)),ExpVal(ValInt(1)))))))); m => (n, (counterFunction (([fun: _+_] n) 1)))]

And  ctrBROKEN2 evaluates to:

  RESULT: (m => (n, (counterFunctionBROKEN2 (([fun: _+_] n) 1))))

Note that counter, (which uses the version of counterFunction that uses ExpAbs), saves the environment mapping but ctrBROKEN2 (which uses the version of counterFunction 
that uses ValAbs) does not save the mapping. Specifically, there is no definition of counterFunctionBROKEN2 in the result for ctrBROKEN2, but there is a defintion of
counterFunctionin the result for counter. 
 
If each is applied to a value, ExpVal(ValNil), as given in vals counterTest and counterTestBROKEN2:

  val counterTest:Exp = ExpLet ("counter", ExpApp (counter, ExpVal (ValNil)), ExpFst (ExpVar ("counter")))
  val counterTestBROKEN2:Exp = ExpLet("counter_BROKEN2", ExpApp (ctrBROKEN2, ExpVal (ValNil)), ExpFst (ExpVar ("counter_BROKEN2")))
 
When counterTest and counterTestBROKEN2, are evaluated, the one using ValAbs (i.e., ctrBROKEN2) fails on applying the function to ValNil because the environment is not saved.  
Specifically, counterFunctionBROKEN2 is not available so there is nothing to apply to ExpVal(ValNil).  This means that the "counter_BROKEN2", has no function to apply to ValNil.
    
The output for running counterTestBROKEN is:

Evaluation of counterTestBROKEN: val counterTestBROKEN:Exp = ExpLet("counter_BROKEN2", ExpApp (ctrBROKEN, ExpVal (ValNil)), ExpFst (ExpVar ("counter_BROKEN2")))
EVALUATING: (let counter_BROKEN2 = (((counterFunctionBROKEN2 n => (m => (n, (counterFunctionBROKEN2 (([fun: _+_] n) 1))))) 4) Nil) in (fst counter_BROKEN2))
ExpLet(counter_BROKEN2,ExpApp(ExpApp(ExpRecAbs(counterFunctionBROKEN2,n,ExpVal(ValAbs(m,ExpPair(ExpVar(n),ExpApp(ExpVar(counterFunctionBROKEN2),ExpApp(ExpApp(ExpVal(ValPrimFun(Some(_+_),<function1>)),ExpVar(n)),ExpVal(ValInt(1)))))))),ExpVal(ValInt(4))),ExpVal(ValNil)),ExpFst(ExpVar(counter_BROKEN2))) --> ?   in environment Map()
ExpApp(ExpApp(ExpRecAbs(counterFunctionBROKEN2,n,ExpVal(ValAbs(m,ExpPair(ExpVar(n),ExpApp(ExpVar(counterFunctionBROKEN2),ExpApp(ExpApp(ExpVal(ValPrimFun(Some(_+_),<function1>)),ExpVar(n)),ExpVal(ValInt(1)))))))),ExpVal(ValInt(4))),ExpVal(ValNil)) --> ?   in environment Map()
ExpApp(ExpRecAbs(counterFunctionBROKEN2,n,ExpVal(ValAbs(m,ExpPair(ExpVar(n),ExpApp(ExpVar(counterFunctionBROKEN2),ExpApp(ExpApp(ExpVal(ValPrimFun(Some(_+_),<function1>)),ExpVar(n)),ExpVal(ValInt(1)))))))),ExpVal(ValInt(4))) --> ?   in environment Map()
ExpRecAbs(counterFunctionBROKEN2,n,ExpVal(ValAbs(m,ExpPair(ExpVar(n),ExpApp(ExpVar(counterFunctionBROKEN2),ExpApp(ExpApp(ExpVal(ValPrimFun(Some(_+_),<function1>)),ExpVar(n)),ExpVal(ValInt(1)))))))) --> ?   in environment Map()
ExpVal(ValInt(4)) --> ?   in environment Map()
ExpVal(ValAbs(m,ExpPair(ExpVar(n),ExpApp(ExpVar(counterFunctionBROKEN2),ExpApp(ExpApp(ExpVal(ValPrimFun(Some(_+_),<function1>)),ExpVar(n)),ExpVal(ValInt(1))))))) --> ?   in environment Map(n -> ValInt(4), counterFunctionBROKEN2 -> ValRecClosure(Map(),counterFunctionBROKEN2,n,ExpVal(ValAbs(m,ExpPair(ExpVar(n),ExpApp(ExpVar(counterFunctionBROKEN2),ExpApp(ExpApp(ExpVal(ValPrimFun(Some(_+_),<function1>)),ExpVar(n)),ExpVal(ValInt(1)))))))))
ERROR:
java.lang.RuntimeException: expected function in application
at interpreter$.error(interpreter.scala:24)
        at interpreter$.evaluate(interpreter.scala:70)
        at interpreter$.evaluate(interpreter.scala:74)
        at interpreter$.testExample(interpreter.scala:434)
        at interpreter$.main(interpreter.scala:461)
<snip>
