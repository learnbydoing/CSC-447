...
object fp2 {
  ...
  def filter [A] (xs:List[A], f:A=>Boolean) : List[A] = {
	...
	case Nil => Nil
       	...
       	case y::ys if(f(y) == false) => filter(ys, f)
      }
  }
  ...
  def joinTerminateLeft (xs : List[String], term : String) : String = {
	...
	case(u::us, term) => foldLeft(u::us, "", (a:String, b:String) => a+b+term )
      }
  }
  ...
  def joinTerminateRight (xs : List[String], delimiter : String) : String = {
    // TODO: Provide definition here.
     (xs, delimiter)  match {
	case (Nil, _) => ""
	case(u::Nil, delimiter) => u+delimiter
	case(u::us,delimiter) => foldRight(u::us, "", (a:String, b:String) => a+delimiter+b)
      }
  }
  ...
  def firstIndexNumGreaterThan (a : Int, xs : List[Int]) : Int = {
    ...
    xs match {
	case Nil => throw new RuntimeException
  	case (y::ys) => if(y>=a) 0 else 1 + firstIndexNumGreaterThan(a, ys)
    } 
  }
  ...
  def allEqual (xs : List[Int]) : Boolean = {
	...
	case Nil => true
	case (y::Nil) => true
 	case (y::ys) => if(y==myHead(ys)) allEqual(ys) else false
    }
  }
  ...
  def stringLengths (xs:List[String]) : List[(String, Int)] = {
    // TODO: Provide definition here.
  	def g(s:String) : (String, Int) = {
		(s, s.length())
	} 
   	xs match {
          case Nil => List()
	  case (y::ys) => (y::ys).map( (s:String) =>  g(s))
    	}
  }
  ...
  def delete1 [X] (x:X, ys:List[X]) : List[X] = {
    // TODO: Provide definition here.
     ys match {
	case Nil => Nil
	case (v::vs) => (if(v==x) delete1(x, vs) else v::delete1(x, vs))
    }
  }
  ...
  def delete2 [X] (x:X, ys:List[X]) : List[X] = {
    ...
    ys match {
	case Nil => Nil
	case(v::vs) => for(q <- vs; if(q!=x)) yield q 
    } 
  }
  ...
  def removeDupes1 [X] (xs:List[X]) : List[X] = {
	...
	case (Nil) => Nil
	case (u::Nil) => List(u)
	case (u::us) => if(u==us.head)removeDupes1(us) else u::removeDupes1(us)
    }
  }
  ...
  def splitAt [X] (n:Int, xs:List[X]) : (List[X], List[X]) = {
	...
	case (0, list)      => (List(), list)
    	case (n, u::us)   => {
		val (p1, p2) = splitAt(n - 1, us)
		if(n>0) (u :: p1, p2) else (p1, u :: p2)
		}
	}
}
  ...
  def allDistinct (xs : List[Int]) : Boolean = {
	...
	case Nil => true
	case (u::Nil) => true
	case (u::us) => if(member(u, us)) false else allDistinct(us)
    }
  }
}
